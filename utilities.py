...
def build_inputs(args, sess):
  if args.overfit:
    ...
    # Regular dataset
    train_filenames = np.array(glob.glob(os.path.join(args.train_dir, '*.png'), recursive=True))
    val_filenames = np.array(glob.glob(os.path.join('../dataset/benchmarks', '**', '*_HR.png'), recursive=True))
    print("train_filenames====================--------------:::",os.path.join(args.train_dir, '*.png'))
    print("Benchmarks====================--------------:::",os.path.join('../dataset/benchmarks', '**', '*_HR.png'))
